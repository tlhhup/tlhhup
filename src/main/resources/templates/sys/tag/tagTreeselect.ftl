...
<head>
    ...
    <script type="text/javascript">
        ...
        var tree, setting = {
            ...
            async: {enable: (type == 3), url: "${ctx}/sys/menu/treeData", autoParam: ["id=officeId"]},
            data: {
                simpleData: {
                    enable: true,
                    idKey: "id",
        ...
        $(document).ready(function(){
            $.get("${ctx}${url}${(url?index_of('?')==-1)?then('?','&')}extId=${extId}&isAll=${isAll}&module=${module}&t="
...
<div id="search" class="form-search hide" style="padding:10px 0 0 13px;">
    ...
    <input type="text" class="empty" id="key" name="key" maxlength="50" style="width:110px;height: 28px;">
